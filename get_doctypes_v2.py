...
def get_data():
    ...
    for m in main_doctypes:
        ...
        # Create directory for each key, to store child doctype data
        for k in keys:
            ...
            for c in m["childs"]:
                ...
                data, keys = result
                # Key on dictionary
                if c["doctype"] in childs_data:
                    childs_data[c["doctype"]].extend(data)
                else:
                    childs_data[c["doctype"]] = data
        
        # Write childs
        for doctype_name, data in childs_data.items():
            all_doctype_data.append(data)
